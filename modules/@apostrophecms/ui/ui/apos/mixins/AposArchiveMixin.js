    ...
    async archive(doc) {
      ...
      try {
        ...
        const isCurrentContext =
          doc.aposDocId === window.apos.adminBar.context.aposDocId;
        const plainType = isPage
          ? this.$t("apostrophe:page")
          : this.$t(moduleOptions.label) || this.$t("apostrophe:document");

        const sentences = [];

        sentences.push(
          this.$t("apostrophe:confirmArchive", {
            type: plainType,
            title: doc.title,
          })
        );
        ...
        if (descendants > 0) {
          sentences.push(
            this.$t("apostrophe:archivingPageHasChild", {
              count: descendants,
            })
          );
        }
        ...
        if (draftDescendants > 0) {
          sentences.push(
            this.$t("apostrophe:archivingDraftChildCount", {
        ...
        if (isPublished) {
          sentences.push(
            this.$t("apostrophe:archivingWillUnpublish", {
        ...
        if (draftDescendants > 0) {
          sentences.push(
            this.$t("apostrophe:archivingWillDeleteDraftChildren")
          );
        }
        ...
        if (isModified) {
          if (isPage) {
            sentences.push(
              this.$t("apostrophe:archivingPageWillLoseDraftChanges")
            );
          } else {
        ...
        const confirm = await apos.confirm({
          ...
          affirmativeLabel: this.$t("apostrophe:archiveTypeAffirmativeLabel", {
            type: plainType,
          }),
    ...
    async restore(doc) {
      ...
      try {
        ...
        if (isPage && doc._children && doc._children.length) {
          ...
          const description = `You are going to restore the ${plainType} “${
            doc.title
          }”, which has ${childLength} child ${plainType}${
            doc._children.length > 1 ? "s" : ""
          }.`;
        ...
        if (AposAdvisoryLockMixin.methods.isLockedError(e)) {
          ...
          await apos.alert({
            heading: this.$t("apostrophe:error"),
            description:
              e.message || this.$t("apostrophe:errorWhileRestoringArchive"),
            localize: false,
          });
        }
      }
